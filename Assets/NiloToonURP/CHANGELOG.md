...
### Breaking change
- In Unity6.4 or above, NiloToon will always use NiloToon's outline bake method instead of Unity's outline bake method, due to copy asset in scriptable importer is not allowed in Unity6.4 anymore

### Added
- support depth only rendering for Render Graph (now support Volumetric Light Beam's shadow, PIDI Planar reflection's depth render)
- support Unity6.4 & 6.5, by removing NiloToon's non-RenderGraph code for Unity6.4 or above. In Unity6.4 or above, you can only use RenderGraph since Unity removed compatibility mode completely.
- NiloToonVolumePresetPicker: +Nilo035-ConcertDarkShadowHighContrast.asset
----------------------------------------------
## [0.17.11] - 2025-12-02

### Added
- NiloToonCharacter shader: +Glitter (Experimental, WIP), for testing if Glitter works on character surface
- NiloToonAnimePostProcessVolume: + toplight receive main light shadow 
- NiloToonBloomVolume, NiloToonCinematicRimLightVolume: +"?" button (url to document)

### Fixed
- fix vrm10 material convert bug (_UvAnimScrollYSpeed,_UvAnimRotationSpeed)
- NiloToonCharacter shader: fix _DissolveMode float->uint, now dissolve works in Unity2021.3
- NiloToonCharacter shader: fix dither fadeout fail to compile bug on iPhone15Pro
----------------------------------------------
## [0.17.10] - 2025-10-27

### Breaking change
- NiloToon character shader: remove the effect of _UnityCameraDepthTextureWriteOutlineExtrudedPosition (will still keep the UI for a while). It is an old function that only works in the pre motion vector era (Unity2021.3), now when TAA/motion blur/DLSS/FSR is much more common, this function brings more harm than benefit in most cases, so we decided to remove it. Now unity's depth texture will always render non-outline character depth no matter _UnityCameraDepthTextureWriteOutlineExtrudedPosition is on or off

### Added
- Add a new UI page "Edit > Project Settings > NiloToon", allow user to control project-wide 'outline bake' and 'shader stripping' options. If you build using command line and require custom shader stripping, you need to use it. 
- Now Parallel import is supported automatically (experimental)
- Volume picker: +033,034 volume (idol bright)
----------------------------------------------
## [0.17.9] - 2025-10-15

### Fixed
- Auto material conversion now works for Material Variant input in Unity2022.3 or above
----------------------------------------------
## [0.17.8] - 2025-10-13
