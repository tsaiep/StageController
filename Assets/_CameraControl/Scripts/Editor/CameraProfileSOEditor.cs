...
public class CameraProfilePreviewRenderer
{
    ...
    private void SetupPreviewCamera(CameraProfileSO profile, float t)
    {
        ...
        {
            float distance = Mathf.Max(general.posDistanceCurve.Evaluate(t), 0.25f);

            Vector3 targetOffset = new Vector3(
                general.posTargetOffsetXCurve.Evaluate(t),
                general.posTargetOffsetYCurve.Evaluate(t),
                general.posTargetOffsetZCurve.Evaluate(t)
            );

            Vector2 screenOffset = new Vector2(
                general.posScreenXCurve.Evaluate(t),
                general.posScreenYCurve.Evaluate(t)
            );
        ...
        if (profile is TrackingProfileSO tracking)
        {
            Vector3 followOffset = new Vector3(
                tracking.followOffsetXCurve.Evaluate(t),
                tracking.followOffsetYCurve.Evaluate(t),
                tracking.followOffsetZCurve.Evaluate(t)
            );
            ...
            Vector3 rotationOffset = new Vector3(
                tracking.rotTargetOffsetXCurve.Evaluate(t),
                tracking.rotTargetOffsetYCurve.Evaluate(t),
                tracking.rotTargetOffsetZCurve.Evaluate(t)
            );
        ...
        if (profile is DollyProfileSO dolly)
        {
            float normalizedPosition = Mathf.Clamp01(dolly.splinePositionCurve.Evaluate(t));
            ...
            Vector3 rotationOffset = new Vector3(
                dolly.rotTargetOffsetXCurve.Evaluate(t),
                dolly.rotTargetOffsetYCurve.Evaluate(t),
                dolly.rotTargetOffsetZCurve.Evaluate(t)
            );
